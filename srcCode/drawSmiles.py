...
from rdkit.Chem.rdCoordGen import AddCoords as addcords
...
def draw_substructure(molecule, info, bit, size, detail_level=1):
    """Draw only the substructure related to the fingerprint bit with adjustable detail."""
    center, _ = info[bit][0]  # Extract center atom
    radius = detail_level  # Use detail_level as the environment radius

    atoms, bonds, atom_colors, bond_colors = get_environment(molecule, center, radius)

    # Create a substructure using only relevant atoms and bonds
    submol = Chem.PathToSubmol(molecule, bonds)
...
def draw_molecule_and_bit_info_grid(molecule, info, bit, size, max_examples=None, mols_per_row=3
    ...
    return Draw.MolsToGridImage(
        ...
        molsPerRow=1,
        subImgSize=(size, size),
        highlightAtomLists=[atoms_to_highlight[0]],
        highlightBondLists=[bonds_to_highlight[0]],
        highlightAtomColors=[atoms_to_highlight_colors[0]],
        highlightBondColors=[bonds_to_highlight_colors[0]],
        drawOptions=options,
        returnPNG = False
    )
